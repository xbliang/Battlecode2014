...
public class MotionUnit {
	...
	private int getMapData(int x, int y) {
		if(x<=0 || y <= 0 || x+2>=map.length|| y+2>=map[0].length) {
			return -1;
		}
		if(map[x+1][y+1] == 0) {
	...
	private void connectMap(int x, int y, int val) { //everything is in game coordinates so we need to do the offset of that
		if(getMapData(x,y) == -1) {
			map[x+1][y+1] = val;
	...
	private boolean clearedObstacle(MapLocation p, MapLocation end,int obstacle) {		
		while(true) {
	...
	private void updateWaypoints(MapLocation currentPos) {
	...
	public void tryNextMove() throws GameActionException  {
		BasicPathing.tryToMove(getNextDirection(rc.getLocation()), false, rc, directionalLooks, allDirections);
	}
}
