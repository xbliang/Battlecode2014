...
public class SoldierController {
	...
	private static int defenseChannel;
	...
	public SoldierController(RobotController r,int t,int fc, int cc,int dc) throws GameActionException {
		rc = r;
	...
	public static void checkDefense() throws GameActionException {
		int d = rc.readBroadcast(focusChannel);
		defense = (d==1); 
	}
	...
	public static double rateMove(MapLocation oldLoc, MapLocation loc) {
		...
		if(defense) {
			if(oldLoc.distanceSquaredTo(target) <64 && loc.distanceSquaredTo(target) > 64){
				return 0;
			}
		}
	...
	private static Direction retreat() throws
	GameActionException {
	...
	/*public static int favorableAdvance(MapLocation loc) {
		...
		for(int j = 0; j < numFriendlySoldiers; j++) {
			RobotInfo finfo = friendlySoldiers[j];
			for(int t = 0; t < numTargets; t++) {
				if(finfo.location.distanceSquaredTo(targets[t]) <= 17 && finfo.health > 10) {
					favorable++;
					break;
				}
			}
	...
	/*public static int favorableShotLocation(MapLocation loc, int reinforceRange, int confidence) {
		numTargets = 0;
		int favorable = 1;
		double remainingHealth = rc.getHealth();
		for(int i = 0; i < numEnemySoldiers; i++) {
		...
		for(int j = 0; j < numFriendlySoldiers; j++) {
			RobotInfo finfo = friendlySoldiers[j];
			for(int t = 0; t < numTargets; t++) {
				if(finfo.location.distanceSquaredTo(targets[t]) <= reinforceRange && finfo.health > 10) {
					++favorable;
					break;
				}
			}
	...
	public void runSoldier() throws GameActionException {
		...
		/*for(int i = -1; i <= 1; i++) {
			for(int j = -1; j <= 1; j++) {
				MapLocation potential = new MapLocation(rc.getLocation().x+i,rc.getLocation().y+j);
				int quality = sdQuality(potential);
				if(quality > 1) {
					System.out.println("dumpster" + quality);
				}
			}
		}*/
