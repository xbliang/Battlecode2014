...
public class SpiralPatternReachable extends NoiseTowerPattern {
	...
	public void searchStep() {
		...
		current = q.poll();
		incoming = f.poll();
		//System.out.println(Clock.getBytecodesLeft() + " " + current);
		previous = current.add(incoming.opposite()); 
		if (!shouldSearch(previous, current, 250)) {
			return;
		}
		//System.out.println("Finished should check");
	...
	public boolean onMap(MapLocation m) {
		return (m.x >= 0 && m.y >= 0 && m.x < mapWidth && m.y < mapHeight);
	}
	...
	public void shootNext() throws GameActionException {
		...
		while (Clock.getBytecodesLeft() > 270 && !doneSearching) {
			searchStep();
		}
	}
