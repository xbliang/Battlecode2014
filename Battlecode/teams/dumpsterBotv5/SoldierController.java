...
public class SoldierController {
	...
	private static int nearbyFriendlies(MapLocation start, RobotInfo[] robots) {
		int close = 0;
		for(RobotInfo r : robots) {
			int d = start.distanceSquaredTo(r.location);
			if(d <= 10) {
	...
	private static MapLocation averageRobotLocation(RobotInfo[] robots) {
		...
		for(RobotInfo r : robots) {
			pos.add(r.location.x, r.location.y);
		}
	...
	public static double rateSquare(RobotInfo[] friendlies,RobotInfo[] enemies, MapLocation loc, double targetUpper, double targetLower) {
		...
		double badness = 0;
		double distFromGroup = distToNearest(loc, friendlies);
		if(myDiff >= threshold) {
			badness += myDiff;
		}
		/*for(RobotInfo info : friendlies) {	
			if(info.location.isAdjacentTo(loc) && badness < 1) {
				badness += 0.5;
			}
		}*/
		
		int near = nearbyFriendlies(loc, friendlies);
		if(near == 0) {
			badness += 10;
		} else if(near == 1) {
			badness += 5;
