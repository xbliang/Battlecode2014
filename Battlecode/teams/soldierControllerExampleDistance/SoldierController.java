...
public class SoldierController {
	...
	private static double formationQuality(RobotInfo[] robotInfo,MapLocation loc,boolean enemies, MapLocation averageEnemy) {
		...
		double quality = 0;
		int num = 1;
		dists[0] = Math.sqrt(loc.distanceSquaredTo(averageEnemy));
		double avgDist = 0;
		double variance = 0;
		for(RobotInfo info : robotInfo) {			
			if(info.type == RobotType.SOLDIER) {
				if(info.team == rc.getTeam()) {
					double d = Math.sqrt(info.location.distanceSquaredTo(averageEnemy));
					dists[num] = d;
					num++;
				}
			}
		...
		for(int i = 0; i < num; i++) {		
			variance += (dists[i]-avgDist)*(dists[i]-avgDist);			
		}
		variance /= num;
		if(num == 0) {
			return 100;
		} else {
			return Math.abs(dists[0]-avgDist);
			//return variance;
		}
	}
	...
	public void runSoldier() throws GameActionException {
		...
		if(highestRating > 0) {
			...
			if(!helpFriends()) {
				double quality = formationQuality(robotInfo, rc.getLocation(), true, averageEnemy);
				rc.setIndicatorString(1, "Form Quality: "+quality);
				if(friendly > 1 && quality >1 ) {				
					Direction best = optimalFormationMove(robotInfo,rc.getLocation(),true, averageEnemy);
					...
					rc.setIndicatorString(2, "Moving to target " + dest);						
					motion.tryNextMove();
				}
			}
		}
