...
public class MotionUnit {
	...
	int lookahead;
	boolean onPath;
	static final int IN_ENEMY_HQ_RANGE = 65536;
	boolean reachable;
	...
	private void blockEnemyHQ() {
		for(MapLocation loc : MapLocation.getAllMapLocationsWithinRadiusSq(rc.senseEnemyHQLocation(), 25)) {
			if(loc.x >= 0 && loc.x < map.length && loc.y >= 0 && loc.y < map[0].length)
				map[loc.x][loc.y] = IN_ENEMY_HQ_RANGE;
		}
	}
	...
	public void updateWaypoints() {
		...
		if(!currentPos.equals(goal)) {
			...
			if(collision) {
				...
				int attempts = 1000;
				if(obs == IN_ENEMY_HQ_RANGE)
					if(map[goal.x][goal.y] == IN_ENEMY_HQ_RANGE)
						return;
					else
						attempts = 15;
