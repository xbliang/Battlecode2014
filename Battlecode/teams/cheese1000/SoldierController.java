...
public class SoldierController {
	...
	public SoldierController(RobotController r,int t) throws GameActionException {
		rc = r;
		buildSpot = new MapLocation(rc.getMapWidth()/2, rc.getMapHeight()/2);
		buildSpot = buildSpot.add(buildSpot.directionTo(rc.senseHQLocation()));
	...
	public void runSoldier() throws GameActionException {
		...
		if(myLoc.distanceSquaredTo(target) < RobotPlayer.SOLDIER_SENSOR_RANGE && (enemy == 0 || friendlyTurns/enemyTurns > 3)){
			MapLocation[] cowSquares = MapLocation.getAllMapLocationsWithinRadiusSq(myLoc, RobotPlayer.SOLDIER_SENSOR_RANGE);
			for(MapLocation loc : cowSquares)
				if(target.distanceSquaredTo(loc) < RobotPlayer.SOLDIER_SENSOR_RANGE && rc.senseCowsAtLocation(loc) > 1000) {
					highestRating = 420;
					highestTarget = loc;
				}
		}
		if(enemy != 0 && (enemyTurns > friendlyTurns + 1/* || !helpNear(averageFriendly,averageEnemy)*/)) {			
			...
			if(noiseBuilder || pastrBuilder) {
				...
				if(rc.isActive() && (currentRound > startRound + 200 || currentRound > 1600)) {
					if(noiseBuilder)
