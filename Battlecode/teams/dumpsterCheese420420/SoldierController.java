...
public class SoldierController {
	...
	public static void checkFocus() throws GameActionException {
		...
		if(focus == 1 && rc.readBroadcast(RobotPlayer.PASTR_URGENCY_CHANNEL) != 1 && rc.readBroadcast(RobotPlayer.NOISE_URGENCY_CHANNEL) != 1) {
			focusOn = true;
		} else {
			focusOn = false;
		}
	...
	private static void updateInformation() throws GameActionException {
		Robot[] nearbyRobots = rc.senseNearbyGameObjects(Robot.class,64);	
		...
		distToNearest = distToNearestSq(rc.getLocation(), enemies, numEnemies);
	...
	public static double rateMove(MapLocation loc) {
		int oldDist = distToNearest;
		int dist = distToNearestSq(loc, enemies,numEnemies);
	...
	private static Direction adjustFormation() throws GameActionException {
		...
		if(focusOn && !healing) {
			...
			for(int look : advanceLooks) {
				...
				if(rc.canMove(d)) {				
					double r = rateMove(rc.getLocation().add(d));
					if(r > best) {
						best = r;
						bestd = d;
					}
			...
			if(!GOINGIN) {
				...
				if(!retreat) {
					if(highestRating > 0) {
						...
						if(numEnemySoldiers > 0 || (numEnemies > 0 && !focusOn)) {
							...
							if(!helpFriends()) {				
								...
								if(rc.isActive() && Clock.getRoundNum() > rc.readBroadcast(RobotPlayer.NOISE_BUILD_CHANNEL) && (rc.getLocation().distanceSquaredTo(buildSpot) < 3 || Clock.getRoundNum() > 1600)) {
									rc.broadcast(RobotPlayer.NOISE_BUILD_CHANNEL, Clock.getRoundNum()+110);
									rc.construct(RobotType.NOISETOWER);
									return;
								}
								else if(rc.isActive() && Clock.getRoundNum() > rc.readBroadcast(RobotPlayer.PASTR_BUILD_CHANNEL) && (rc.getLocation().distanceSquaredTo(buildSpot) < 5 || Clock.getRoundNum() > 1600)) {
									rc.broadcast(RobotPlayer.PASTR_BUILD_CHANNEL, Clock.getRoundNum()+55);
									rc.broadcast(RobotPlayer.PASTR_LOCATION_CHANNEL, Util.locToInt(rc.getLocation()));
									rc.construct(RobotType.PASTR);
									return;
								}
								if(rc.getLocation().distanceSquaredTo(buildSpot) < 69)
									motion.tryNextSneak();
								else
									motion.tryNextMove();
							}
