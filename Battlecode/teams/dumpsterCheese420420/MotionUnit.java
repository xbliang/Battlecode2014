...
public class MotionUnit {
	...
	static StackOfMapLocations path;
	static MapLocation goal;
	static int obstacle_number;
	static int lookahead;
	static boolean onPath;
	static boolean targetInEnemyHQ;
	static boolean toBuildSpot;
	...
	public void startPath(MapLocation dest) throws GameActionException {
		path = new StackOfMapLocations();
		goal = dest;
		MapLocation buildSpot = Util.intToLoc(rc.readBroadcast(RobotPlayer.FIRST_PASTR_CHANNEL));
		toBuildSpot = goal.equals(buildSpot);
		path.push(goal);
		onPath = true;	
	...
	public void updateWaypoints() {
		...
		if(!currentPos.equals(goal)) {
			...
			if(collision) {
				...
				for(int x = 0; x < 200; x++) {
					if(x == 199) { //if takes too long must be disconnected
						path = new StackOfMapLocations();
						path.push(goal);
						onPath = true;	
						targetInEnemyHQ = true;	
						if(targetInEnemyHQ)
							map = mapHQ;
						else
							map = mapNoHQ;
	...
	public void tryNextMove() throws GameActionException {
		if(toBuildSpot && rc.readBroadcast(RobotPlayer.MATRIX_CHANNEL+Util.locToInt(rc.getLocation())) != 0) {
			int minDist = 99999;
			Direction move = null;
			for(Direction dir : allDirections) {
				MapLocation landing = rc.getLocation().add(dir);
				int dist = rc.readBroadcast(RobotPlayer.MATRIX_CHANNEL+Util.locToInt(landing));
				if(rc.canMove(dir) && dist != 0 && landing.distanceSquaredTo(rc.senseEnemyHQLocation()) > 25) {
					if(dist < minDist) {
						minDist = dist;
						move = dir;
					}
			...
			if(move != null) {
				//rc.setIndicatorString(0,"A STARRING TO " + move);
	...
	public void tryNextSneak() throws GameActionException {
		if(toBuildSpot && rc.readBroadcast(RobotPlayer.MATRIX_CHANNEL+Util.locToInt(rc.getLocation())) != 0) {
			...
			if(move != null)
				BasicPathing.tryToSneak(move, false, rc, directionalLooks, allDirections);
			return;
		}
		updateWaypoints();
		//rc.setIndicatorString(0, ""+path.peek());
	...
	public Direction getNextDirection() throws GameActionException {
		...
		updateWaypoints();
		Direction dir = rc.getLocation().directionTo(path.peek());
		return BasicPathing.nextDirection(dir, false, rc, directionalLooks, allDirections);
	}
}